...
cargo-lite-lib: src src/lib.rs
	(test -e cargo-lite.conf && clear && echo "--- The file 'cargo-lite.conf' already exists") || (echo -e "deps = [\n]\n\n[build]\ncrate_root = \"src/lib.rs\"\ncrate_type = \"library\"\nrustc_args = []\n" > cargo-lite.conf && clear && echo "--- Created 'cargo-lite-conf' for library" && cat cargo-lite.conf)

rust-ci-lib: src src/lib.rs
	(test -e .travis.yml && clear && echo "--- The file '.travis.yml' already exists") || (echo -e "before_install:\n\t- yes | sudo add-apt-repository ppa:hansjorg/rust\n\t- sudo apt-get update\ninstall:\n\t- sudo apt-get install rust-nightly\nscript:\n\t- make lib\n" > .travis.yml && clear && echo "--- Created '.travis.yml' for library" && cat .travis.yml)
...
src/test.rs:
	test -e src/test.rs || (touch src/test.rs)

src/lib.rs:
	test -e src/lib.rs || (echo -e "#[crate_id = \"\"];\n#[deny(missing_doc)];\n\n//! Documentation goes here.\n" > src/lib.rs)

clean:
	rm -rf build/
	rm -rf doc/
	rm -f bin/*
	clear && echo "--- Deleted binaries and documentation"
